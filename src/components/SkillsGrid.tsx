...
import { 
  SiPython, SiPhp, SiJavascript, SiTypescript, SiFastapi, SiSpringboot, SiLaravel, SiFlask, SiDjango, SiExpress, SiNodedotjs,
...
import { FaRobot, FaBrain, FaTerminal, FaJava, FaLanguage } from "react-icons/fa6";
...
const getSkillIcon = (skillName: string) => {
  ...
  if (s.includes("english") || s.includes("japanese") || s.includes("英語") || s.includes("日本語")) return <FaLanguage className="w-7 h-7 text-indigo-500" />;
  ...
  if (s.includes("node")) return <SiNodedotjs className="w-7 h-7 text-[#339933]" />;
  ...
  if (s.includes("linux") || s.includes("bash") || s.includes("n-tier")) return <SiLinux className="w-7 h-7 dark:text-white" />;
      ...
      >
        ...
        <div className="flex flex-col gap-14">
          {CATEGORIES.map((category, catIndex) => {
            ...
            const skills = t.raw(`items.${category}`) as string[];
            ...
            return (
              ...
              >
                <h4 className="text-xl font-semibold text-zinc-800 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800 pb-2 inline-block self-start">
                  {categoryTitle}
                </h4>
                <div className="flex flex-wrap gap-4 sm:gap-5">
                  {skills.map((skill, index) => (
                    ...
                    >
                      ...
                      {/* Tooltip */}
                      <span className="absolute -bottom-7 opacity-0 group-hover:opacity-100 transition-opacity text-xs font-mono text-zinc-500 dark:text-zinc-400 whitespace-nowrap pointer-events-none z-10">
                        {skill}
                      </span>
                    </motion.div>
                  ))}
